...
用户可以维护一组历史盈亏调整记录。所有记录的金额之和构成账户级“历史实际总盈亏偏移值”，并入跨账户累计投资结果。系统只定义聚合算法，不限制用户记录的数据来源、业务含义或是否存在重复。

已确认的第一版约束：

- 每条记录包含 CNY 金额、备注、创建时间和更新时间，并允许新增、编辑和删除。
- `历史实际总盈亏偏移值 = SUM(历史盈亏调整记录金额)`；没有记录时为 `0`。
...
- 偏移值仅支持人民币 CNY；表单不提供币种或汇率选项，不保存旧账户的原始币种、折算汇率、汇率日期或业务发生日期。
- 当前净投入小于或等于 `0` 时，历史总收益率显示“已回本”，暂不计算回本后的复杂收益率。

> **不完整方案声明**：仅支持 CNY 是为了以最低成本覆盖当前个人使用需求，并不构成完整的多币种历史业绩模型。旧账户使用 USD、HKD 等币种时，用户需自行按认可的汇率折算为 CNY；系统不记录原币、折算汇率或汇率日期，也不会随汇率变化重估该偏移值。若未来需要审计级跨账户收益或多币种归因，应另行设计，不能在此字段上继续叠加隐式语义。
