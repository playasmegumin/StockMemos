...
### Milestone 8: 持仓可视化 & K 线增强 📅（规划中）
- [ ] 持仓概览标签占比统计（按主标签聚合，环形图 + 表格）
- [ ] K 线图止盈止损水平虚线
...
### Milestone 10: 平台可用性监测 📅（规划中）
- [ ] 外部服务状态诊断页（数据源延迟/可用性、API Key 状态）
...
- 统计方式：按主标签聚合，一只股票有多个标签时取第一个（添加顺序）
- 图表类型：环形图 + 明细表格（待定）
- 数据范围：持仓市值（数量 × 现价），按标签汇总
